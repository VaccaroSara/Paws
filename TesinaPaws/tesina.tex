...
Il progetto consiste nello sviluppo di \textbf{Paws}, un'applicazione mobile pensata per gli amanti degli animali, i privati e le strutture d'accoglienza (canili e rifugi). L'obiettivo dell'applicazione è offrire uno strumento integrato ed intuitivo per facilitare l'adozione e la condivisione di annunci relativi a cuccioli ed animali domestici, tenendo traccia delle schede di dettaglio dei singoli animali e consentendo il contatto diretto tra gli utenti.

Il sistema prevede due macro-tipologie di utenti:
\begin{itemize}
    \item \textbf{Utenti Privati:} appassionati ed individui che utilizzano l'applicazione per consultare gli annunci di cuccioli disponibili, salvare gli animali preferiti nella propria sezione dedicata, pubblicare nuovi annunci di adozione, filtrare la ricerca per razza, età, sesso e tipologia di utente, e contattare direttamente i proprietari tramite chiamata telefonica.
...
\begin{itemize}
    \item \textbf{Applicazione Android Nativa:} sviluppata in Kotlin sfruttando l'architettura nativa Android e la libreria di componenti UI, garantendo elevate prestazioni, gesture fluide (swipe-to-favorite) e perfetta integrazione con i servizi di sistema come l'avvio delle chiamate ed i servizi Firebase (Authentication, Firestore e Storage).
    \item \textbf{Applicazione Clone Cross-Platform:} realizzata in Flutter per consentire la portabilità del codice a partire da un'unica codebase, replicando fedelmente le funzionalità, la gerarchia tipografica (serif/sans-serif/custom font), l'identità visiva e l'interfaccia dell'applicazione nativa.
\end{itemize}
